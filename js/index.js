...
(function () {
  ...
  // Show empty text in favorites section if there are no movies
  ...
  searchKeyword.addEventListener("keyup", function () {
    ...
    if (search === "") {
      ...
      suggestionList = []; // Clears the previous movies from the array
    } else {
  ...
  // Add movie to the suggestion container in the DOM
  ...
  async function handleFavBtn(e) {
    ...
    // Check if the movie is already present in the favorite list
    let isPresent = false;
